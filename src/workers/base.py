...
class BaseWorker(ABC):
    ...
    async def start(self) -> None:
        ...
        self._running = True
        self._log.info("Воркер %s подключается к очереди %s...", self.name, self.queue_name)
        ...
        try:
            ...
            async with queue.iterator() as queue_iter:
                async for message in queue_iter:
                    if not self._running:
                        self._log.info("Остановка прослушивания воркером %s", self.name)
                        break
                    ...
                    async with message.process():
                        ...
                        try:
                            ...
                            self._log.exception("Ошибка при обработке сообщения в воркере %s: %s", self.name, e)
            ...
            self._log.exception("Критическая ошибка в воркере %s: %s", self.name, e)
        finally:
            self._running = False
