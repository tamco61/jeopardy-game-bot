...
class LobbyHandler:
    ...
    async def handle_ready(self, chat_id: int, player_id: str, username: str, is_ready: bool) -> None:
        try:
            await self._ready.execute("room_1", player_id, is_ready=is_ready)
            status = "готов" if is_ready else "не готов"
            await self._tg.send_message(chat_id, f"Игрок @{username} {status}!")
        except Exception as e:
            await self._tg.send_message(chat_id, f"Ошибка: {e}")

    async def handle_leave(self, chat_id: int, player_id: str, username: str) -> None:
        try:
