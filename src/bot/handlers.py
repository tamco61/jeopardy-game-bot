    ...
    async def handle_update(self, update: dict) -> None:
        ...
        if callback_query is not None:
            ...
            if result.captured:
                ...
                q_text = room_now.current_question.text if room_now and room_now.current_question else "Вопрос"
                await self._tg.edit_message_text(
                    chat_id=chat_id,
                    message_id=message_id,
                    text=f"Вопрос: {q_text}\n\n🛑 Отвечает @{username}! Ждем ответ в чат.",
                )
                await self._tg.answer_callback_query(callback_query["id"], text="Твой ответ!")
            else:
